...
def filetopdf():
        if filepath != None:
            ...
            if(filename.lower().endswith('.docx')):
                docxTOpdf()
        if spath is None:
            messagebox.showerror("Warning", "You must choose a save distination first")
...
def imgtopdf():
        ...
        if(name is None):
            ...
            os.close()
        if(spath is None):
            messagebox.showerror("Warning", "You must choose a path to save the file")
...
def docxTOpdf():
        ...
        name = nn 
        if spath != None:
            newf = str.join(spath,name)
            convert(r'filepath','{newf}.pdf')
        if(name is None):
            warning()
            os.close()
        else: 
